...
export function AdyenTestConnectionDialog({
  ...
  const buildSteps = React.useCallback((): TestStep[] => {
    ...
    return [
      step('reach', t`Reach Adyen API`),
      step('auth', t`Authenticate credentials`),
      step('environment', t`Verify environment`),
      step('merchant', t`Fetch merchant account`)
    ];
  }, [t]);
  ...
  const mockErrorFor = React.useCallback(
    (stepId: string): string => {
      switch (stepId) {
        case 'reach':
          return t`Could not reach the Adyen API — request timed out after 5000 ms.`;
        case 'auth':
          return t`401 Unauthorized — the API key was rejected.`;
        case 'environment':
          return t`Environment mismatch — these credentials belong to a different environment.`;
        case 'merchant':
          return t`Merchant account not found — check the Merchant ID.`;
        default:
          return t`No response from Adyen.`;
      }
    },
    [t]
  );
  ...
  const runTests = React.useCallback(async () => {
    ...
    for (const [index, stepDef] of initialSteps.entries()) {
      ...
      const stepPassed = Math.random() >= 0.3;
      if (!stepPassed) {
        failed = true;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="top-[15vh] translate-y-0">
        <DialogHeader>
          ...
          <DialogDescription>
            <Trans>
              Checks the connection to Adyen using the current form values.
            </Trans>
          </DialogDescription>
        </DialogHeader>
...
function EndpointNode({
  ...
  return (
    <div className="flex w-16 shrink-0 flex-col items-center gap-2 text-center sm:w-24">
      <div className="flex size-11 items-center justify-center rounded-xl border border-border/60 bg-card text-foreground/70 dark:bg-input/30">
        {icon}
      </div>
