...
export const reservations = pgTable(
  'reservations',
  {
    ...
    bookingFrom: timestamp('booking_from', {
      withTimezone: true,
      mode: 'string'
    }),
  ...
  () => [
    check(
      ...
      sql`state = ANY (ARRAY['pending'::text, 'started'::text, 'done'::text])`
    ),
    check(
      'reservations_check_in_via_check',
      sql`check_in_via = ANY (ARRAY['android'::text, 'ios'::text, 'tv'::text, 'station'::text, 'web'::text])`
    ),
    check(
      'reservations_check_out_via_check',
      sql`check_out_via = ANY (ARRAY['android'::text, 'ios'::text, 'tv'::text, 'station'::text, 'web'::text])`
    ),
    check(
      'reservations_purpose_check',
      sql`purpose = ANY (ARRAY['private'::text, 'business'::text])`
    )
  ]
);
